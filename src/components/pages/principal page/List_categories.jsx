...
export const List_categories = ({ funcDeleteCategory }) => {
  ...
  return (
    <div className="w-full flex flex-wrap justify-center overflow-auto relative p-4">
      ...
      {categories?.map((category) => (
        <div
          key={category?._id}
          className="flex flex-col my-5 m-3 sm:mx-5 w-5/12 h-[180px] max-w-[150px] relative bg-gradient-to-bl from-secondary to-primary rounded-md shadow-lg hover:cursor-pointer hover:scale-105   hover:shadow-primary_detail/10 hover:shadow-xl transition-all duration-150 ease-linear"
        >
          <div className="h-full bg-white/5 rounded-md"></div>
          <h3 className="w-full text-center text-primary_detail text-sm font-serif py-1 font-semibold">
            {category?.name}
          </h3>
          <button
            onClick={(e) => {
